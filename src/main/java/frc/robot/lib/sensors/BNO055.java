...
public class BNO055 extends GyroBase {
	...
	// How long the last I2C read of data took (expect less than .003 seconds)
	private double readDurationLast;
	
	// The maximum time it took for an I2C read of data (should be less than .005 seconds)
	private double readDurationMax;
	
	// If I2C read takes longer than this, flag it
	private static final double READ_TOO_LONG_THRESHOLD = 0.05;
	...
	private void calculateVector() {
		...
		if (readDurationLast > readDurationMax) {
			readDurationMax = readDurationLast;
		}
	 ...
	 * 
	 * @return Returns the heading from the gyro in the range of 0 - 360 degrees.
	 */
	 ...
	 * 
	 * @return Returns the roll from the gyro in the range of -90 to +90 degrees.
	 */
	...
	/**
	 * Get the pitch (z axis) of the sensor in non-continuous format (-180 - +180).
	 * 
	 * @return Returns the pitch from the gyro in the range of -180 to +180 degrees.
	 */
	 ...
	 * 
	 * @param verbosity Controls how much stuff to show (-1 nothing, 0 add heading, ... 9 adds everything).
	 */
	public void updateDashboard(int verbosity) {
		if (verbosity < 0) {
			return;
		}
		
		boolean present = isSensorPresent();
		if (verbosity >= 4) {
			SmartDashboard.putBoolean("BNO055 Present", present);
		}
		if (present == false) {
			return;
		}
		...
		if (verbosity >= 3) {
			SmartDashboard.putBoolean("BNO055 Initialized", initialized);
		}
		if (initialized == false) {
			return;
		}
		
		double[] gyro = getVector();
		SmartDashboard.putNumber("BNO055 Heading", getHeading());
		
		if (verbosity < 1) {
			return;
		}

		SmartDashboard.putNumber("BNO055 X", gyro[0]);
		SmartDashboard.putNumber("BNO055 Y", gyro[1]);
		SmartDashboard.putNumber("BNO055 Z", gyro[2]);
